...
public:
    ...
    Trade match_one_sell(Order& incoming_sell) {
        if (incoming_sell.side != Side::Sell || buys_.empty()) {
            return {};
        }

        const Price best_price = buys_.begin()->first;
        if (incoming_sell.price > best_price) {
            return {};
        }
        ...
        const Trade trade{resting_buy.id, incoming_sell.id, resting_buy.price, traded};
